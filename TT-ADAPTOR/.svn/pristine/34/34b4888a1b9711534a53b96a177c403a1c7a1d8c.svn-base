...
public class TradeWithCibcRfsQuoteHandler {
	private final static Logger logger = LoggerFactory.getLogger(TradeWithCibcRfsQuoteHandler.class);
	...
	public TradeWithCibcRfsQuoteHandler(Transaction transactionMessage, IMkQfixApp qfixApp, IResponseDialectHelper dialect, LatencyVo latencyRecord, ICertifiedPublishingEndpoint certifiedPublishingEndpoint) {
...
//		String valueDate = transactionDetail.getValueDate();
		...
		if ( !transactionMessage.getOrderParams().hasQuoteRefId() || transactionMessage.getOrderParams().getQuoteRefId().length() == 0) {
			logger.debug("Rejecting Trade as no QuoteRefId");
			rejectTrade();
		} else {
