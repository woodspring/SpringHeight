package com.tts.reuters.adapter.utils;
...
import com.tts.reuters.adapter.ReutersRFAAdapterPriceReq;
 ...
 */
public final class ReutersGenericOMMParser
{
     ...
     */
    public synchronized static void initializeDictionary(String fieldDictionaryFilename,
    ...
    public synchronized static FieldDictionary getDictionary(int dictId)
    {
        if (dictId == 0)
            dictId = 1;
     ...
     */
    public synchronized static final void parse(OMMMsg msg)
    {
        parseMsg(msg, System.out);
    }

    private synchronized static final String hintString(OMMMsg msg)
    {
    ...
    private synchronized static boolean append(StringBuilder buf, String str, boolean first)
    {
        if (!first)
        {
            buf.append(" | ");
     ...
     * PrintStream
     */
    public synchronized static final void parseMsg(OMMMsg msg, PrintStream ps)
    {
        parseMsg(msg, ps, 0);
    }

    public synchronized static final void parseMsg(OMMMsg msg, PrintStream ps, int tabLevel)
    {
        msg.getMsgType();
     ...
     * PrintStream
     */
    public synchronized static final void parseDataDefinition(OMMDataDefs datadefs, short dbtype,
    ...
    private synchronized static void dumpRespTypeNum(OMMMsg msg, PrintStream ps)
    {
     ...
     */
    public synchronized static final void parse(OMMData data)
    {
        parseData(data, System.out, 0);
    }
    
    private static boolean contains(String fieldName) {
    	for(ReutersRFAAdapterPriceReq.INTERESTED_FIELDS field : ReutersRFAAdapterPriceReq.INTERESTED_FIELDS.values()) {
    ...
    public synchronized static final Hashtable<String, String> parseAggregate(OMMData data)
    {
        Hashtable<String, String> priceData = new Hashtable<>();        
        ...
        try	{
	        ...
	        for(Iterator iter = ((OMMIterable)data).iterator(); iter.hasNext();)
	        {
	            ...
	            if(entry instanceof OMMFieldEntry) {
	            	OMMFieldEntry fe = (OMMFieldEntry) entry;
	            	FidDef fidDef    = CURRENT_DICTIONARY.getFidDef(fe.getFieldId());
	            	
	            	if(contains(fidDef.getName()))	{           	
	            		String pirceKey  = fidDef.getName();
			            
			            OMMData priceVal = ((OMMFieldEntry) entry).getData(fidDef.getOMMType());
			            priceData.put(pirceKey, priceVal.toString());
	            	}
	            }
	        }
        }
    ...
    private synchronized static final void parseAggregate(OMMData data, PrintStream ps, int tabLevel)
    {
     ...
     * PrintStream
     */
    public synchronized static final void parseData(OMMData data, PrintStream ps, int tabLevel)
    {
        if (data.isBlank())
            dumpBlank(ps);
    ...
    private synchronized static final void parseAggregateHeader(OMMData data, PrintStream ps, int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        short dataType = data.getType();
        ...
        switch (dataType)
        {
            ...
            case OMMTypes.MAP:
            {
                ...
                String flagsString = ReutersGenericUtil.mapFlagsString(s);
                dumpIndent(ps, tabLevel);
                ps.print("flags: ");
            ...
            case OMMTypes.VECTOR:
            {
                ...
                String flagsString = ReutersGenericUtil.vectorFlagsString(s);
                dumpIndent(ps, tabLevel);
                ps.print("flags: ");
                ps.println(flagsString);
            ...
            case OMMTypes.FILTER_LIST:
            {
                ...
                String flagsString = ReutersGenericUtil.filterListFlagsString(s);
                dumpIndent(ps, tabLevel);
                ps.print("flags: ");
                ps.println(flagsString);
            }
    ...
    private synchronized static final void dumpBlank(PrintStream ps)
    {
        ps.println();
    }

    private synchronized static final void dumpIndent(PrintStream ps, int tabLevel)
    {
        for (int i = 0; i < tabLevel; i++)
            ps.print('\t');
    }

    private synchronized static final void parseEntry(OMMEntry entry, PrintStream ps, int tabLevel)
    {
        try
        {
    ...
    private synchronized static final void dumpEntryHeader(OMMEntry entry, PrintStream ps, int tabLevel)
    {
        dumpIndent(ps, tabLevel);
    ...
    private synchronized static final void dumpFieldEntryHeader(OMMFieldEntry entry, FidDef def, PrintStream ps,
            int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        ps.print(OMMTypes.toString(entry.getType()));
    ...
    private synchronized static final void dumpElementEntryHeader(OMMElementEntry entry, PrintStream ps,
            int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        ps.print(OMMTypes.toString(entry.getType()));
    ...
    private synchronized static final void dumpFilterEntryHeader(OMMFilterEntry entry, PrintStream ps,
            int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        ps.print(OMMTypes.toString(entry.getType()));
        ...
        String flagsString = ReutersGenericUtil.filterEntryFlagsString(entry);
        dumpIndent(ps, tabLevel);
        ps.print("flags: ");
        ps.println(flagsString);
    ...
    private synchronized static final void dumpMapEntryHeader(OMMMapEntry entry, PrintStream ps, int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        ps.print(OMMTypes.toString(entry.getType()));
        ps.print(" (");
        ...
        String flagsString = ReutersGenericUtil.mapEntryFlagsString(entry);
        dumpIndent(ps, tabLevel);
        ps.print("flags: ");
        ps.println(flagsString);
    ...
    private synchronized static final void dumpVectorEntryHeader(OMMVectorEntry entry, PrintStream ps,
            int tabLevel)
    {
        dumpIndent(ps, tabLevel);
        ps.print(OMMTypes.toString(entry.getType()));
        ...
        String flagsString = ReutersGenericUtil.vectorEntryFlagsString(entry);
        dumpIndent(ps, tabLevel);
        ps.print("flags: ");
        ps.println(flagsString);
    ...
    public synchronized static final void parseAnsiPageData(OMMData data, PrintStream ps, int tabLevel)
    {
