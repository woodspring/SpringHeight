...
public class MasSchedulingWorkerImpl implements ISchedulingWorker {
	
	private static final Logger logger = LoggerFactory.getLogger(ISchedulingWorker.class);
	private final ScheduledExecutorService service;

	public MasSchedulingWorkerImpl(ScheduledExecutorService executorService ) {
	...
	public ScheduledFuture<?> scheduleAtFixedRate(Runnable runnable, long period) {
		...
		return service.scheduleAtFixedRate(new ExceptionWrappedRunnable(runnable), 0, period, TimeUnit.SECONDS);
	}
