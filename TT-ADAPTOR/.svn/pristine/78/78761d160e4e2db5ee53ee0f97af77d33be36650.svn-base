...
public final class ReutersGenericUtil
{
    ...
    public static void dumpIntList(String tag,int[] input)
    {
    	_sb.setLength(0);
    ...
    public static void dumpLongList(String tag,long[] input)
    {
    	_sb.setLength(0);
	...
	public static void dumpline( int space, String s )
	{
	...
	public static boolean isSuspect( OMMMsg ommMsg )
	{
		...
		if( ommMsg.getState().getStreamState() == OMMState.Stream.OPEN && 
            ommMsg.getState().getDataState() == OMMState.Data.SUSPECT )
		{
			return true;
		}
		else
	...
	public static boolean isValidStreamingRefresh( OMMMsg ommMsg )
	{
		if( ommMsg.has(OMMMsg.HAS_STATE) )
		{
			if( ommMsg.getState().getStreamState() == OMMState.Stream.OPEN && 
	...
	public static boolean isValidSnapshotImage( OMMMsg ommMsg )
	{
		if( ommMsg.has(OMMMsg.HAS_STATE) )
		{
			if( ommMsg.getState().getStreamState() == OMMState.Stream.NONSTREAMING && 
				ommMsg.getState().getDataState() == OMMState.Data.OK )
			{
				return true;
			}
	...
	public  static String ommStateString( OMMState state )
	{
		String s = String.format("OMMState: Stream: %s, Data: %s, Code: %s, Text: %s\n",
	...
	public static void dumpLine( String s )
	{
		System.out.println( s );
	}

	public static void dumpText( String s )
	{
		System.out.print( s );
	}
}
