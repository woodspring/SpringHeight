...
public class DefaultROEAppImpl extends AbstractSubscribingApp implements IFixListener	{
	public final static String NAME_TTS_ROE  = "ROE_IN_FIXADP_DFLT";
	...
	public DefaultROEAppImpl(IMkQfixApp qfixApp, ISchedulingWorker worker, SessionInfo sessionInfo,
		...
		AppUtils.createCustomRollingFileAppender("server-FixRoeDefault." + AppUtils.getAppName() + ".log", "FIXROELogger", AppUtils.LOGLEVEL__DEBUG);
	...
	private void doROEActiveOrderDataMigration()	{
		...
		logger.info("MARKET_ROE_MIGRATE_ORDER - " + migrateAllOrders);
				
		fixLoggedOn = ((sendOrderTarget != null) && (!sendHeatBandTarget.trim().isEmpty()) && (qfixApp.isLoggedOn(sendOrderTarget)));
			...
			if(topic.equals(IEventMessageTypeConstant.RestingOrderMgmt.RR_TRAN_ROR_TO_MR_ORDER))	{
				...
				if(RestingOrderSubmitType.SUBMIT.equals(submitType) && roeMessage != null)	{
					...
					if(isSessionConnected)	{
						...
						logger.warn("ORDER_ID: " + roeMessage.getOrderId() + ", CREATION ORDER REJECTED:SESSION OFFLINE.");
					...
					if(isSessionConnected)	{
						...
						logger.warn("ORDER_ID: " + roeMessage.getOrderId() + ", CANCEL ORDER REJECTED:SESSION OFFLINE.");
						broadcastUpdatedOrder(ttsRequestId, roeMsgBuilder);
					}
				}
	...
	public String[] getDesiredTopics() {
		return new String[] { IEventMessageTypeConstant.RestingOrderMgmt.RR_TRAN_ALL_ORDER, 
							  IEventMessageTypeConstant.Transactional.TRAN_ROR_TEMPERATURE };
	}
	...
	public void onMessage(Message message, IQfixRoutingAgent routingAgent)
		...
		if((message != null)  && (message instanceof quickfix.fix44.ExecutionReport))
			orderId = responseDialectHelper.convertRestOrderResponseAndUpdate((quickfix.fix44.ExecutionReport) message, roeBuilder);
		...
		if((message != null)  && (message instanceof quickfix.fix44.OrderCancelReject))	
			orderId = responseDialectHelper.convertRestOrderResponseAndUpdate((quickfix.fix44.OrderCancelReject) message, roeBuilder);
