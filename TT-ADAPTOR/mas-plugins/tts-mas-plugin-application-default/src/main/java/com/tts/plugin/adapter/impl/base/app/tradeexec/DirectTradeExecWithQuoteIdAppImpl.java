...
public class DirectTradeExecWithQuoteIdAppImpl extends AbstractSubscribingApp implements IEspPriceRepoDependent {
	private final static Logger logger = LoggerFactory.getLogger(DirectTradeExecWithQuoteIdAppImpl.class);

	public static final String MY_NAME = "TRADE_ADAPTER";
	
	private final boolean espPriceCheck;
	...
	public DirectTradeExecWithQuoteIdAppImpl(
			IMkQfixApp qfixApp,
	...
	public void onRequest(String topic, TtMsg ttMsg) {
		...
		try {
			...
			if ( (transactionMessage.hasQuoteRefId() && transactionMessage.getOrderParams().getQuoteRefId().startsWith(IFixConstants.ESP_QUOTE_REF_ID_PREFIX)) ) {
